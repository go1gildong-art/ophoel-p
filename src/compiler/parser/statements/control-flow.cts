...
export const controlFlow: ActionMap<Statement | any> = {
    IfStatement(_if, cond, body, elifs, elseClause) {
        ...
        const elifSignatures = elifs.toAST(__filename).map((elif: any) => ({
            condition: elif.condition.toAST(__filename),
            body: elif.body.toAST(__filename)
        }));

        const elseSignature = elseClause.toAST(__filename)[0] ? {
            condition: elseClause.toAST(__filename)[0].body.toAST(__filename),
            body: elseClause.toAST(__filename)[0].body.toAST(__filename)
        } : undefined;
    ...
    WhileStatement(_while, condition, body) {
        return new ASTs.WhileStatement(
            condition.toAST(__filename),
            body.toAST(__filename),
            getLoc(_while, __filename)
        );
    },

    ForStatement(_for, _open1, declaration, condition, _semi, increment, _close1, body) {
        return new ASTs.ForStatement(
            declaration.toAST(__filename),
            condition.toAST(__filename),
            increment.toAST(__filename),
            body.toAST(__filename),
            getLoc(_for, __filename)
        );
    },

    ForOfStatement(_for, _open1, declaration, _of, iterable, _close1, body) {
        return new ASTs.ForOfStatement(
            declaration.toAST(__filename),
            iterable.toAST(__filename),
            body.toAST(__filename),
            getLoc(_for, __filename)
        );
    },
    ...
    RepeatStatement(_repeat, count, body) {
        return new ASTs.RepeatStatement(
            count.toAST(__filename),
            body.toAST(__filename),
            getLoc(_repeat, __filename)
        );
    },
